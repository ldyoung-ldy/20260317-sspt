...
1. **跑 `/review` 收尾 Step 2 diff** — 补齐结构性问题检查，重点看 slug、时间窗口、server actions 错误处理
2. **补 authenticated admin `/qa` 负向场景** — 非法时间、重复评分维度、`select` 空选项、取消发布回收首页曝光
3. **决定 slug 策略** — 明确是否接受中文赛事名生成 `/2026` 这类结果，否则调整 slug 规则
4. **补编辑页 / 删除策略设计** — 明确 Step 2 是否要做到完整 CRUD，避免带着能力缺口进入 Step 3
5. **同步 acceptance 最终结论** — 把 review / qa 的通过项、未覆盖项、已知风险补齐到 `acceptance/`
6. **决定验收测试数据去留** — 当前数据库中保留了一条已发布的验收赛事，需要确认是否回收

## 进度总览

```
  Step 1: 项目脚手架        [x] 12/12  ██████████  Day 1  ← 已完成
  Step 2: 赛事管理          [~] 11/14  ████████░░  Day 2-3
...
> 优先级: P0 | 预估: 1.5 天 | 状态: 🚧 进行中（主链路已打通，待 review / qa 收尾）
...
- [ ] 2.12 `/review` 收尾：补一次 diff review，重点检查 slug、时间窗口、server action 错误处理
- [ ] 2.13 `/qa` 收尾：补测取消发布、非法时间、重复评分维度、`select` 空选项、slug 边界
- [ ] 2.14 编辑页 / 删除策略：确认是否在进入 Step 3 前补完整 CRUD

### Step 2 当前进度说明

- 已通过导入 `localhost` 管理员登录态完成后台手工验收闭环：创建赛事 → 发布 → 首页可见 → 详情页可见
- `acceptance/step-2-events-checklist.md` 与 `acceptance/step-2-events-manual-script.md` 已写入本次实测记录
- 赛事管理主链路已经可用，但编辑页、删除策略、负向校验 QA 仍未收尾

### Step 2 当前风险 / 阻塞

- **slug 可读性风险**：中文赛事名当前可能生成过短 slug（本次验收样例为 `/2026`）
- **数据库稳定性风险**：验收期间出现过一次 Neon 瞬时不可达，虽已恢复，但仍需关注超时与重试策略
- **验收数据残留**：数据库中存在已发布的验收赛事 `管理员流程回归赛 2026`，需决定是否保留
- **计划与实现不一致**：旧计划默认 Step 2 未开始，但实际主链路已完成，后续应以“收尾与补证据”为主
